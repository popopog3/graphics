...
void keyboard(unsigned char key, int x, int y)
{
	...
	switch (key) {
	case 'a':
		g_camera.rotate_left(0.5);
		break;
	case 'd':
		g_camera.rotate_right(0.5);
		break;
	}
...
void special(int key, int x, int y)
{
	...
	switch (key) {
	case GLUT_KEY_UP:
		g_camera.move_forward(0.5);
		break;
	case GLUT_KEY_DOWN:
		g_camera.move_backward(0.5);
		break;
	case GLUT_KEY_RIGHT:
		g_camera.move_right(0.5);
		break;
	case GLUT_KEY_LEFT:
		g_camera.move_left(0.5);
		break;
	default:
		break;
	}
